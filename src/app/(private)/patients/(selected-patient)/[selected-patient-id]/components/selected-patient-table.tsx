...
import { SelectedPatientTableItem } from "./selected-patient-table-row";
import { SelectedPatientsTableHeader } from "./selected-patients-table-header";

type SortableColumn = "ID" | "TITLE" | "DATE" | "DURATION" | "STATUS" | null;

type SortDirection = "ASC" | "DESC" | null;

export function SelectedPatientsTable() {
  const {
    appointments,
          ...
          {isGettingAppointments
            ...
            : !isGettingAppointments && appointments.length !== 0
              ? appointments.map((row) => (
                  <SelectedPatientTableItem key={row.id} appointment={row} />
                ))
              : !isGettingAppointments &&
